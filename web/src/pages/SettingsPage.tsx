...
export const SettingsPage = () => {
  ...
  const toggle2FAMutation = useMutation({
    mutationFn: async (enabled: boolean) => {
      await apiClient.post('/user/toggle-2fa', { enabled });
    },
    onSuccess: (_, enabled) => {
      queryClient.invalidateQueries({ queryKey: ['user-config'] });
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div className="space-y-4">
                  ...
                  {userConfig?.twoFactorEnabled && (
                    <Button variant="ghost" className="w-full text-[10px] uppercase tracking-widest font-bold border-primary/20 text-primary hover:bg-primary/10">
